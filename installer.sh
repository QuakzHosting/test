...
if [[ -f "./installed" ]]; then
    ...
    curl -sSLo ubuntu-base.tar.gz http://cdimage.ubuntu.com/ubuntu-base/releases/20.04/release/ubuntu-base-20.04.4-base-amd64.tar.gz >/dev/null 2>err.log
    echo -ne '#####               (25%)\r'
    
    echo "Extracting Ubuntu base system..."
    mkdir -p ./ubuntu-root
    tar -xzf ubuntu-base.tar.gz -C ./ubuntu-root >/dev/null 2>err.log
    echo -ne '##########           (50%)\r'
    ...
    echo -ne '\n'
    echo "
${bold}${lightgreen}Ubuntu environment successfully installed!
========================================================================
 "
    function runcmd1 {
        printf "${bold}${lightgreen}Ubuntu${nc}@${lightblue}Container${nc}:~ "
        read -r cmdtorun
        ./libraries/proot -S ./ubuntu-root /bin/bash -c "$cmdtorun"
        runcmd
    }
    function runcmd {
        printf "${bold}${lightgreen}Ubuntu${nc}@${lightblue}Container${nc}:~ "
